...
public class UIDrawCall : MonoBehaviour
{
    ...
    public static System.Func<string, Shader> externalLoader;
    private Shader LoadShader(string path)
    {
        Shader s = null;
        if (externalLoader != null)
        {
            s = externalLoader(path);
        }
	...
	void CreateMaterial ()
	{
		...
		{
			mTextureClip = true;
			shader = LoadShader("Hidden/" + shaderName + textureClip);
		}
		else if (mClipCount != 0)
		{
			shader = LoadShader("Hidden/" + shaderName + " " + mClipCount);
			if (shader == null) shader = LoadShader(shaderName + " " + mClipCount);

			// Legacy functionality
			if (shader == null && mClipCount == 1)
			{
				mLegacyShader = true;
				shader = LoadShader(shaderName + soft);
			}
		}
		else shader = LoadShader(shaderName);

		// Always fallback to the default shader
		if (shader == null) shader = LoadShader("Unlit/Transparent Colored");
	...
	static UIDrawCall Create (string name)
	{
        ...
        if(root == null)
        {
            ...
            root = UnityEditor.EditorUtility.CreateGameObjectWithHideFlags("UIDrawCallRoot",
...
#else
            root = new GameObject("UIDrawCallRoot");
#endif
