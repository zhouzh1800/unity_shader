...
static public class NGUIText
{
	...
	static public GlyphInfo GetGlyph (int ch, int prev, float fontScale = 1f)
	{
		...
		{
            int readSize = finalSize;
            bool isDynamic = dynamicFont.dynamic;
            if(!isDynamic)
            {
                readSize = dynamicFont.fontSize;
                if(readSize <= 0)
                {
                    isDynamic = true;
                }
            ...
            if (dynamicFont.GetCharacterInfo((char)ch, out mTempChar, readSize, fontStyle))
			{
 #if UNITY_4_3 || UNITY_4_5 || UNITY_4_6 || UNITY_4_7
                ...
                else
                {
                    ...
                    float scale = ((float)finalSize / readSize);
                    glyph.v0.x = mTempChar.minX * scale;
                    glyph.v1.x = mTempChar.maxX * scale;
                    glyph.v0.y = mTempChar.maxY * scale - baseline;
                    glyph.v1.y = mTempChar.minY * scale - baseline;
                }
